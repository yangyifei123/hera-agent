...
describe("createTeamTools (integration)", () => {
  ...
  async function makeAgent(name: string, mode: "primary" | "subagent" | "all" = "subagent") {
    await agentTools.hera_create_agent.execute({
      name, description: `${name} agent`, prompt: `You are ${name}.`, mode,
    } as any, {} as any);
  }
  ...
  describe("hera_list_teams + hera_delete_team", () => {
    it("lists empty, then created, then deleted", async () => {
      ...
      await teamTools.hera_create_team.execute({
        name: "solo-team", description: "x", coordination: "parallel",
    ...
    it("hera_delete_team returns error for unknown team", async () => {
      const result = await teamTools.hera_delete_team.execute({ name: "no-such" } as any, {} as any);
  ...
  describe("hera_get_team_progress", () => {
    it("returns team info for an existing team", async () => {
      ...
      await teamTools.hera_create_team.execute({
        name: "info-team", description: "Info test", coordination: "adaptive",
        members: [{ agent_name: "planner", role: "planner" }],
      } as any, {} as any);
      const result = await teamTools.hera_get_team_progress.execute({ team_name: "info-team" } as any, {} as any);
    ...
    it("returns error for unknown team", async () => {
      const result = await teamTools.hera_get_team_progress.execute({ team_name: "ghost-team" } as any, {} as any);
      expect(String(result)).toContain("not found");
    });
  });
  ...
  describe("hera_export_team", () => {
    it("exports an existing team as a plugin package", async () => {
      ...
      await teamTools.hera_create_team.execute({
        name: "export-team", description: "Export test", coordination: "parallel",
        members: [
      ...
      const pluginDir = join(harness.ctx.paths.configRoot, "agents", "hera-generated", "export-team-team");
    ...
    it("rejects export of unknown team", async () => {
      const result = await teamTools.hera_export_team.execute({ team_name: "ghost" } as any, {} as any);
      expect(String(result)).toContain("not found");
    });
  });
});
